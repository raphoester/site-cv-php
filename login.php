<!DOCTYPE html>
<html lang="en">

<head>

  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta name="description" content="">
  <meta name="author" content="">

  <title>Connexion - espace administration</title>
  <?php header("Cache-Control:no-cache"); ?>

  <!-- Bootstrap core CSS -->
  <link href="vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">

  <!-- Custom fonts for this template -->
  <link href="https://fonts.googleapis.com/css?family=Saira+Extra+Condensed:500,700" rel="stylesheet">
  <link href="https://fonts.googleapis.com/css?family=Muli:400,400i,800,800i" rel="stylesheet">
  <link href="vendor/fontawesome-free/css/all.min.css" rel="stylesheet">

  <!-- Custom styles for this template -->
  <link href="css/resume.min.css" rel="stylesheet">

</head>

<?php
var_dump($_COOKIE);
if (!empty($_COOKIE["connecte"]))
  {
    header("Location: admin.php");
    exit();
  }

if (!empty($_POST["mdp"])) {
    $pdo = new PDO("mysql:host=localhost;dbname=curriculum_vitae", "root", "" , array(PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION));
    $result = $pdo->query("SELECT mdp FROM utilisateur;");
    $motdepasse = $result->fetch(PDO::FETCH_OBJ);
    if ($motdepasse->mdp == md5($_POST["mdp"]))
    {
        setcookie("connecte", " ", time()+5000000, '/');
        header("Location: admin.php");
        exit();
    }

    // var_dump($_COOKIE);
    // url : ebd20f5dd2bcff2044e9e37c97a6c70e.php
}
?>



<body id="page-top">
    <form action="" method = "POST" class = "form-info">
    <div class="form-group">
        <label for="contenu">Mot de passe</label>
        <input type="password" class="form-control" id="mdp" name="mdp">
    </div>
    <div class="form-group">
    <button type="submit" class="btn btn-primary">Connexion</button>
    </div>
    </form>

    <hr class="m-0">
  <!-- Bootstrap core JavaScript -->
  <script src="vendor/jquery/jquery.min.js"></script>
  <script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

  <!-- Plugin JavaScript -->
  <script src="vendor/jquery-easing/jquery.easing.min.js"></script>

  <!-- Custom scripts for this template -->
  <script src="js/resume.min.js"></script>

</body>

</html>
